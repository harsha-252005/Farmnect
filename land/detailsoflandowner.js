...
const fname=localStorage.getItem("farmername");
      const fphone=localStorage.getItem("phone");
    ...
    const db=getDatabase(app);
      ...
      document.getElementById("displayPhone").innerHTML=fphone;
      document.getElementById("displayAddress").innerHTML=add;
      document.getElementById("confirmbutton").addEventListener("click",function()
      {
        const actiondb1=ref(db,`landactions/${farmerid}/${key}`);
              const updatedData = {
              status: "confirmed to purchase" 
              };
              
             
             
              update(actiondb1, updatedData)
       window.location.href='termscon.html'
      })